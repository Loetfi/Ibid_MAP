    ...
    <div id="wrapper">
        ...
        <div class="wrapper wrapper-content">
			...
			<?php include('header.php');?>
				...
				//ambil_item.php
				?>
				<form action="ambil_item.php" method="get" class="form-inline">		
				<div class="col-md-12">
					<div class="ibox float-e-margins">
						...
						<div class="col-md-5">
						 ...
						 <button class="btn btn-primary btn-block pull-right" type="submit">Search</button>	
                 ...
                 <table class="footable table table-stripped" data-page-size="20" data-filter=#filter>
                          <thead>
                            <tr>
                              ...
                              <th width="5%">Year</center></th>
                              <th width="15%">Make,Model<BR>Cylinder, Grade</th>
                              <th width="10%">Auction Day, Km</th>
                              <th width="10%">Colour</th>
							  <th width="5%">Transmision<BR>Fuel</th>
                              <th width="5%">Grade</th>
                              <th width="10%">Start<BR>Price</th>
							  <th width="10%">Market<BR>Price</th>
							</tr>
                          </thead>
                          <tbody>
                            <?php 
							$qsearch="select distinct(idauction_item) as idauction_item,no_pol,title,merk,seri,cylinder,grade,score,score_in,auc_location,id_schedule,
							id_biodata,idauction_item,auc_location,auc_date,year,color,mission,score,fuel,km,minimum_price,final_price,image_url from map_items_history where score_ex='SEARCH' and merk='$make' and seri='$seri' and cylinder='$cc' and grade='$gr' and score='$sc' group by idauction_item order by merk,seri,cylinder,grade,model limit 50";
							$rsearch=mysql_query($qsearch);
							$num=mysql_num_rows($rsearch);
							if ($num<=0){
							...
							while ($row=mysql_fetch_array($rsearch))
							{
								...
								$gr2=$row['grade'];
								$sgr2=$row['score_in'];
								$jadwal=$row['id_schedule'];
								$item2=$row['idauction_item'];
								
								$qmarket="select min(final_price) as harga1 from map_items_history where merk='$merk2' and seri='$seri2' and cylinder='$cc2' and grade='$gr2' and score='$score' and final_price>0";
								$rmarket=mysql_query($qmarket);
								$row_map=mysql_fetch_array($rmarket);
								$harga1=$row_map['harga1'];
								
								$qmarket1="select max(final_price) as harga2 from map_items_history where merk='$merk2' and seri='$seri2' and cylinder='$cc2' and grade='$gr2' and score='$score'";
								$rmarket1=mysql_query($qmarket1);
								$row_map1=mysql_fetch_array($rmarket1);
								$harga2=$row_map1['harga2'];
							?>
								<tr>
								  <td><a href="map_lot_list_detail.php?id=<?php echo $jadwal;?>&item=<?php echo $item2;?>" target="_blank"><center><img class="mySlides" src="<?php echo $row['image_url'];?>" style="width:50%" onerror="this.src='upload/error.png'"></center></a></td>
